"""Integration tests validating BMOPSO on pymoo-binary-problems suite."""
...
def test_bmopso_on_custom_binary_problem() -> None:
    """Verify that BMOPSO solves custom BinaryProblem implementations."""
...
def test_bmopso_on_mkp() -> None:
    """Validate BMOPSO execution on the Multiple Knapsack Problem (MKP)."""
    ...
    algorithm = BMOPSO(n_particles=20, w=0.5, c1=1.5, c2=1.5)
...
def test_bmopso_on_mubqp() -> None:
    """Validate BMOPSO execution on Multiobjective Unconstrained Binary Quadratic Problem (MUBQP)."""
    problem = MUBQP.from_random(n_var=15, n_obj=2, density=0.7, seed=42)
    algorithm = BMOPSO(n_particles=20, w=0.6, c1=1.4, c2=1.4)
...
def test_bmopso_on_moscp() -> None:
    """Validate BMOPSO execution on Multiobjective Set Covering Problem (MOSCP)."""
...
def test_bmopso_on_mstsp() -> None:
    """Validate BMOPSO execution on Multiobjective Traveling Salesman Problem (MSTSP)."""
    dist_mat1 = np.array([
        [0.0, 2.0, 9.0, 10.0],
        [1.0, 0.0, 6.0, 4.0],
        [15.0, 7.0, 0.0, 8.0],
...
def test_bmopso_on_mofs() -> None:
    """Validate BMOPSO execution on Multiobjective Feature Selection (MOFS)."""
    ...
    algorithm = BMOPSO(n_particles=15, w=0.5, c1=1.5, c2=1.5)
