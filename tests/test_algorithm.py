"""Integration and unit tests for BMOPSO with pymoo."""
...
def test_binary_mopso_initialization() -> None:
    """Verify default and custom hyperparameters of BMOPSO."""
    algo_default = BMOPSO()
...
def test_binary_mopso_mutation_operator() -> None:
    ...
    algo = BMOPSO(n_particles=30, mutation_rate=0.05)
...
def test_binary_mopso_linear_inertia_decay() -> None:
    ...
    algo = BMOPSO(n_particles=20, w_max=0.9, w_min=0.4)
...
def test_binary_mopso_velocity_initialization() -> None:
    ...
    algo = BMOPSO(n_particles=50, v_max=v_max)
    algo.setup(problem)
    algo._initialize()
...
def test_binary_mopso_optimization() -> None:
    """Validate BMOPSO execution integrated with the pymoo framework."""
    n_vars = 12
    problem = SimpleBinaryProblem(n_var=n_vars)
    algorithm = BMOPSO(n_particles=20, w=0.5, c1=1.5, c2=1.5, n_grid=20)
...
def test_adaptive_hypercube_grid_calculation() -> None:
    ...
    from bmopso.util.grid import AdaptiveGrid
...
def test_bmopso_archive_pruning_with_max_archive_size() -> None:
    ...
    problem = SimpleBinaryProblem(n_var=20)
    max_archive = 8
    algo = BMOPSO(n_particles=40, max_archive_size=max_archive, n_grid=10)
...
def test_constrained_dominance_deb_rules() -> None:
    ...
    from bmopso.util.dominance import dominates, find_non_dominated_constrained
