"""Binary Multi-Objective Particle Swarm Optimization (BMOPSO) for pymoo.

Main optimization algorithm module implementing BMOPSO by combining:
...
from bmopso.operators.mutation import apply_mutation
from bmopso.operators.pbest import update_personal_bests
from bmopso.operators.sampling import sample_binary_positions
from bmopso.operators.velocity import update_velocity
from bmopso.util.archive import NonDominatedArchive

__all__ = ["BMOPSO"]


class BMOPSO(Algorithm):

    """Binary Multiobjective Particle Swarm Optimization (BMOPSO) Algorithm.

    The BMOPSO algorithm was originally proposed by Luciano S. de Souza, Péricles B. C. de Miranda,
    ...
    It synthesizes foundational pillars of swarm intelligence and multiobjective optimization:
    1. Binary PSO (BPSO) (J. Kennedy and R. C. Eberhart, 1997): Continuous velocity to binary position
       mapping via the logistic sigmoid activation function.
    2. MOPSO (C. A. Coello Coello, G. T. Pulido, and M. S. Lechuga, 2004): External Pareto archive
       with Adaptive Hypercube Grid partitioning, hypercube fitness-based leader selection,
       crowded hypercube capacity pruning, and non-linear mutation/turbulence.
    3. Constrained-Dominance Principle (K. Deb, 2002): Constraint handling for inequality constraints.
    ...
    **kwargs : Any
    ...
    def _next(self) -> None:
        """Execute one evolutionary step of BMOPSO."""
        if (
            self.X is None
            or self.V is None
